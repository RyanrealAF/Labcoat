...
async function validateSchema(db: D1Database): Promise<void> {
  try {
    ...
    if ((error as any).message?.includes('no such table: schema_version')) {
  ...
  async fetch(request: Request, env: Env): Promise<Response> { 
    ...
    if (url.pathname === '/api/decode' && request.method === 'GET') {
      try {
        ...
        const contentResult = await env.D1.prepare(
          "SELECT tactical_concept FROM lessons WHERE title = ?"
        ).bind(topic).first<{ tactical_concept: string }>();
        ...
        return Response.json(
          { error: 'Decoding failed', details: (error as Error).message },
